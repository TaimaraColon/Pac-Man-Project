...
Map* MapBuilder::createMap(ofImage mapImage){
	...
	int dotCount = 0; //Counter to know when the powerups will appear
	// int max_dots = 0;
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
			...
			/*
				Adding the powerUps:
					- Every 35 dots, a cherry will appear.
					- Every 101 dots, a strawberry will appear.  
					- Every 16 dots, an apple will appear. 
			*/
			
			//Part of the code that was changed
			else if(currentPixel == dotC){
				if(dotCount % 35 == 0){
					CherryPowerUp* cherry = new CherryPowerUp(xPos, yPos, pixelMultiplier, pixelMultiplier);
					cherry->setPlayer(mapInCreation->getPlayer());
					mapInCreation->addEntity(cherry);
					dotCount++;
				}
				//Commented For later implementation

				else if(dotCount % 101 ==0){
					StrawberryPowerUp* strawberry = new StrawberryPowerUp(xPos, yPos, pixelMultiplier, pixelMultiplier); 
					mapInCreation->addEntity(strawberry); 
					dotCount++; 
				}
				else if(dotCount %16 == 0){
					PowerUp* apple = new ApplePowerUp(xPos, yPos, pixelMultiplier, pixelMultiplier);
					mapInCreation->addEntity(apple); 
					dotCount++; 
				}

				//Add additional powerUps here (USE ELSE IFS)

				else {
                	Dot* dot = new Dot(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
				// // if(max_dots++ < 25)               //for testing
